...
function setConnectPos(o,f) {
    ...
    this.parse = function(text) {
        ...
        var m_wiki = /^<?nowiki>/;
        var m_sep = /^---*$/ ;
        var m_conn = /^([u|d|l|r|p][0-9]*)?(<)?==?(?:\[([A-z0-9-_]+)\])?\s*([u|d|l|r|p][0-9]*)?\s*(?:\(([^"=>]*)\))?==?(>)?([u|d|l|r|p][0-9]*)?\[([A-z0-9-_]+)\]\s*$/i ;
        var m_ulink= /\?\[(.+)\]\s*(?:\(([^ ")]+)\s*(?:"(.+)")?\))?/i;
        ...
        function pbox(b) {
            ...
            for(var i in b.bl) {
                ...
                if(cl.match("shift")){
                  ...
                  comp2=comp[1].split(",");
                  //alert(comp[1]);
                  shift_x=comp2[0].replace("\(","");
                  //alert(shift_x);
                  shift_y=comp2[1].replace("\)","");
                  //alert(shift_y);

                }else{shift_x=1;shift_y=1;}
                ...
                if(m_sep.exec(cl)) {
                    ...
                    conn.push( {from:b.id,to:a[8],param:{
                        ...
                        s_pos:(fp+(l.length+((b.title!=null)?1:0))),e_pos:tp,cls:a[5],arrow:ar, gene:a[3], gpos:a[4],shx:shift_x,shy:shift_y}}) ;
